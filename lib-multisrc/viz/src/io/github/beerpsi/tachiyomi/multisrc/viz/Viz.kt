...
open class Viz(
    ...
    private fun commonFormBodyBuilder() =
                            ...
                            "$apiUrl/manga/entitled"
                                    ...
                                    .apply {
                                        ...
                                        addQueryParameter(
                                                ...
                                                vizAndroidAppVersionCode.toString()
                                        )
                                    }
                                    .build()
                                    .toString()

                    val entitledResponse =
                            client.newCall(GET(entitledUrl, headers))
                                    .execute()
                                    .parseAs<EntitledResponseDto>()

                    val validFromField =
                            SubscriptionInfoDto::class.java.getField(
                                    "${subscriptionInfoPrefix}valid_from"
                            )
                    val validToField =
                            SubscriptionInfoDto::class.java.getField(
                                    "${subscriptionInfoPrefix}valid_to"
                            )

                    val validFromRaw =
                            validFromField.get(entitledResponse.subscription_info) as String?
    ...
    override fun fetchPopularManga(page: Int): Observable<MangasPage> {
        return if (page == 1) {
            client.newCall(popularMangaRequest(page)).asObservableSuccess().map {
                popularMangaParse(it)
            }
        } else {
    ...
    override fun popularMangaRequest(page: Int) =
            GET("$apiUrl/manga/store_cached/$vizAppId/$vizDeviceId/$vizVersionId", headers)
    ...
    private fun directoryFromResponse(response: StoreResponseDto): List<MangaSeriesDto> {
        return response.data.filterIsInstance<MangaSeriesWrapperDto>().mapNotNull {
            if (it.manga_series.show_chapter &&
                            (it.manga_series.num_chapters_free > 0 || premiumEnabled)
            ) {
                it.manga_series
            } else {
                null
    ...
    private fun parseDirectory(page: Int): MangasPage {
        ...
        val endRange =
                ((page * 24) - 1).let { if (it <= directory.lastIndex) it else directory.lastIndex }
    ...
    override fun fetchLatestUpdates(page: Int): Observable<MangasPage> {
        return if (page == 1) {
            client.newCall(latestUpdatesRequest(page)).asObservableSuccess().map {
                latestUpdatesParse(it)
            }
        } else {
            Observable.just(parseDirectory(page))
        }
    ...
    override fun fetchSearchManga(
        ...
        return if (page == 1) {
            client.newCall(searchMangaRequest(page, query, filters)).asObservableSuccess().map {
                    response ->
    ...
    private fun searchMangaParse(response: Response, query: String): MangasPage {
        ...
        directory =
                if (query.startsWith("id:")) {
                    ...
                    directoryFromResponse(data).filter {
                        it.title_sort.contains(query, ignoreCase = true) ||
    ...
    override fun fetchMangaDetails(manga: SManga): Observable<SManga> {
        ...
        if (coverArtPreference == DEFAULT) {
            ...
            return client.newCall(chapterListRequest(manga)).asObservableSuccess().map {
                val data = it.parseAs<StoreResponseDto>().data
                val volumes =
                        data.filterIsInstance<MangaWrapperDto>()
                                .mapNotNull { manga ->
                                    if (manga.manga.volume != null &&
                                                    !manga.manga.thumburl.isNullOrEmpty()
                                    ) {
                                        manga.manga
                                    } else {
                                        null
                ...
                manga.thumbnail_url =
                        if (coverArtPreference == FIRST_VOLUME) {
                            volumes[0].thumburl
                        } else {
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        return data.filterIsInstance<MangaWrapperDto>()
                .mapNotNull {
                    val freeChapter =
                            (it.manga.epoch_exp_date == null ||
                                    it.manga.epoch_exp_date > currentEpoch) &&
                                    (it.manga.epoch_pub_date == null ||
                                            it.manga.epoch_pub_date < currentEpoch)
                    if (it.manga.chapter != null && (freeChapter || premiumEnabled)) {
                        val chapterNumber = it.manga.chapter.replace(".0", "")
                        SChapter.create().apply {
                            url =
                                    "/${it.manga.series_vanityurl}-chapter-$chapterNumber/chapter/${it.manga.id}?action=read#${it.manga.numpages}"
                            name = "Chapter $chapterNumber"
                            date_upload =
                                    if (it.manga.publication_date != null) {
    ...
    override fun fetchPageList(chapter: SChapter): Observable<List<Page>> {
        ...
        val request =
                POST(
                        "$apiUrl/manga/auth",
                        headers,
                        commonFormBodyBuilder().apply { add("manga_id", id) }.build(),
                )
        return client.newCall(request).asObservableSuccess().map {
            pageListParse(it, id, pageCount)
        }
    }
    ...
    private fun pageListParse(response: Response, mangaId: String, pageCount: Int): List<Page> {
        ...
        return (0..pageCount).map { Page(it, "https://api.viz.com/manga/get_manga_url#$mangaId") }
    }
                ...
                ListPreference(screen.context).apply {
                    ...
                    setOnPreferenceChangeListener { _, newValue ->
                        val res =
                                preferences
                                        .edit()
                                        .putString(COVER_ART_PREF, newValue.toString())
                                        .commit()
                        res
                    }
                }
    ...
    private fun getEpoch(date: String, defaultValue: Long = 0L): Long =
            kotlin
                    .runCatching {
                        dateFormatter.parse(
